...
class MindfulMedia_Shortcodes {
    ...
    public function taxonomy_archive_shortcode($atts) {
        ...
        foreach ($terms as $term) {
            ...
            // If protected and no access, show locked placeholder
            if ($is_protected && !$has_access) {
                echo '<div class="mm-playlist-locked">';
                echo '<a href="' . esc_url($term_link) . '" class="mm-playlist-locked-link">';
                echo '<div class="mm-playlist-locked-content">';
                echo '<svg width="48" height="48" viewBox="0 0 24 24" fill="currentColor"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z"/></svg>';
                echo '<p>' . __('This playlist is password protected', 'mindful-media') . '</p>';
                echo '<span class="mm-playlist-locked-btn">' . __('Enter Password to View', 'mindful-media') . '</span>';
                echo '</div>';
                echo '</a>';
                echo '</div>';
                echo '</section>';
    ...
    private function render_playlists_section_with_videos($atts) {
        ...
        foreach ($terms as $term) {
            ...
            // If protected and no access, show lock placeholder
            if ($is_protected && !$has_access) {
                $output .= '<div class="mm-playlist-locked">';
                $output .= '<div class="mm-playlist-locked-icon">';
                $output .= '<svg width="48" height="48" viewBox="0 0 24 24" fill="currentColor"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z"/></svg>';
                $output .= '</div>';
    ...
    private function render_filter_chips() {
        ...
        $output .= '<button type="button" class="mm-chip active" data-filter-type="all" data-filter-value="">';
        ...
        if ($show_type) {
            ...
            if (!empty($types) && !is_wp_error($types)) {
                foreach ($types as $type) {
                    ...
                    $output .= '<button type="button" class="mm-chip" data-filter-type="media_type" data-filter-value="' . esc_attr($type->slug) . '">';
                    $output .= $icon;
        ...
        if ($playlist_count > 0) {
            $output .= '<button type="button" class="mm-chip" data-filter-type="playlists" data-filter-value="all">';
        ...
        if (!empty($teachers) && !is_wp_error($teachers)) {
            ...
            foreach ($teachers as $teacher) {
                $output .= '<button type="button" class="mm-chip" data-filter-type="media_teacher" data-filter-value="' . esc_attr($teacher->slug) . '">';
                $output .= $teacher_icon;
        ...
        if (!empty($topics) && !is_wp_error($topics)) {
            ...
            foreach ($topics as $topic) {
                $output .= '<button type="button" class="mm-chip" data-filter-type="media_topic" data-filter-value="' . esc_attr($topic->slug) . '">';
                $output .= $topic_icon;
        ...
        if ($show_duration) {
            ...
            if (!empty($durations) && !is_wp_error($durations)) {
                ...
                foreach ($durations as $duration) {
                    $output .= '<button type="button" class="mm-chip" data-filter-type="media_duration" data-filter-value="' . esc_attr($duration->slug) . '">';
        ...
        if ($show_year) {
            ...
            if (!empty($years) && !is_wp_error($years)) {
                ...
                foreach ($years as $year) {
                    $output .= '<button type="button" class="mm-chip" data-filter-type="media_year" data-filter-value="' . esc_attr($year->slug) . '">';
                    $output .= $year_icon;
